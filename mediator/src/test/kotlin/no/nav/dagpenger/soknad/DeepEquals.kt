...
object DeepEquals {
    fun assertDeepEquals(one: Any?, other: Any?, ignoreFieldNames: Set<String> = setOf("versjon")) {
        ModelDeepEquals(ignoreFieldNames).assertDeepEquals(one, other, "ROOT")
    }
}

private class ModelDeepEquals(val ignoreFieldNames: Set<String>) {
    val checkLog = mutableListOf<Pair<Any, Any>>()
    fun assertDeepEquals(one: Any?, other: Any?, fieldName: String) {
        if (fieldName in ignoreFieldNames) return
