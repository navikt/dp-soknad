...
class SøknadPostgresRepository(private val dataSource: DataSource) :
    ...
    override fun hentPåbegynteSøknader(prosessversjon: Prosessversjon): List<Søknad> {
        return using(sessionOf(dataSource)) { session ->
            session.run(
                queryOf( //language=PostgreSQL
                    """
                    SELECT uuid, tilstand, spraak, sist_endret_av_bruker, soknad_v1.opprettet, person_ident, versjon
                    FROM  soknad_v1
    ...
    override fun lagre(søknad: Søknad) {
        ...
        using(sessionOf(dataSource)) { session ->
            session.transaction { transactionalSession ->
                val dbVersjon = transactionalSession.hentVersjon(visitor.søknadId, visitor.ident)
    ...
    private fun Session.hentVersjon(søknadsId: UUID, ident: String) = run(
        queryOf( // language=PostgreSQL
            "SELECT versjon FROM soknad_v1 WHERE uuid = :uuid AND person_ident = :ident",
            mapOf(
                "uuid" to søknadsId,
                "ident" to ident
            )
            ...
            queries.add(
                queryOf(
                   ...
                   ON CONFLICT(uuid) DO UPDATE SET tilstand=:tilstand,
                                                   ...
                                                   soknadmal=(SELECT id
                                                              FROM soknadmal
                                                              WHERE prosessnavn = :prosessnavn AND prosessversjon = :prosessversjon),
